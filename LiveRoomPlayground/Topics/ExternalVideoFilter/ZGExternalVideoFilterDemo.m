...
- (void)startPreview {
    if ([self.delegate respondsToSelector:@selector(getPlaybackView)]) {
        ...
        avConfig.fps = 30;
        avConfig.videoCaptureResolution = CGSizeMake(720, 1280);
        avConfig.videoEncodeResolution = CGSizeMake(720, 1280);
        [self.zegoApi setAVConfig:avConfig];
...
- (void)startPublish {
    ...
    BOOL publishResult = [self.zegoApi startPublishing:self.streamID title:nil flag:ZEGO_JOIN_PUBLISH];
    if (publishResult) {
...
//        ZGLogInfo(@"推流质量更新：分辨率：%dx%d, 帧率：%ffps, 码率：%fkbps", quality.height, quality.width, quality.fps, quality.kbps);
    if ([self.delegate respondsToSelector:@selector(onExternalVideoFilterPublishQualityUpdate:)]) {
        NSString *qualityString = [NSString stringWithFormat:@"分辨率：%dx%d \n帧率：%.2f fps \n码率：%.2f kbps", quality.width, quality.height, quality.fps, quality.kbps];
